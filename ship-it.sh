...
# - Secondary regions from secondary-regions.txt
# - Optional: SKIP_TESTS=1
...
# Function to get "id region" lines
get_machines() {
  if command -v jq >/dev/null 2>&1; then
    ${FLY} machine list --app "${APP_NAME}" --json 2>/dev/null \
      | jq -r '.[] | "\(.id) \(.region)"' | grep -E '^[0-9a-f]+ [a-z0-9]{3}$' || true
  else
    # Fallback: parse tabular output; keep hex IDs length>=12, find 3-letter region token
    ${FLY} machine list --app "${APP_NAME}" 2>/dev/null | \
      awk '
        {
          # collect potential id (field with only hex, length>=12)
          for(i=1;i<=NF;i++){
            if($i ~ /^[0-9a-f]+$/ && length($i) >= 12){
              id=$i
            }
            if($i ~ /^[a-z0-9]{3}$/){
              region=$i
...
if [ -z "${MACHINES}" ]; then
  echo "ERROR: Could not parse any machines after deploy."
  exit 1
fi

# Base machine: prefer one already in primary region; else first
BASE_ID=$(echo "${MACHINES}" | awk -v pr="${PRIMARY_REGION}" '$2==pr {print $1; exit}')
[ -z "${BASE_ID}" ] && BASE_ID=$(echo "${MACHINES}" | head -1 | awk '{print $1}')

if ! echo "${BASE_ID}" | grep -qE '^[0-9a-f]{12,}$'; then
  echo "ERROR: Derived base machine id '${BASE_ID}' looks invalid."
...
# Clone for each secondary region if missing
if [ -n "${SECONDARY_REGIONS}" ]; then
  while read -r region; do
    ...
    # Refresh machine list after each clone
    MACHINES="$(get_machines)"
